#!/usr/bin/env python3
"""
Generate sprint-status.yaml from OpenProject work packages via MCP.
This script integrates OpenProject MCP to pull epics and stories and generate BMAD sprint status file.
"""
...
def get_openproject_data():
    """
    Fetch work packages from OpenProject via MCP.
    This function will be called by the MCP tool execution.
    """
    # Note: This will be executed via MCP tool calls
...
def generate_sprint_status(work_packages_data, output_file):
    ...
    for wp in work_packages_data.get("work_packages", []):
        ...
        status_id = wp.get("status", {}).get("id", 71)  # Default to "New" (71)
        wp_type = wp.get("type", {}).get("name", "")
        
        # Skip test tasks or non-epic/story items
        ...
        # Check if epic (Summary task type or subject starts with "Epic")
    ...
    with open(output_path, 'w') as f:
        ...
        f.write("# - Status synced from OpenProject MCP\n")
        f.write("\n")
...
if __name__ == "__main__":
    ...
    print("Use the MCP tool to fetch OpenProject data and generate sprint-status.yaml")
