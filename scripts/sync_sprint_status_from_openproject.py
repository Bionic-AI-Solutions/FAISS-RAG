#!/usr/bin/env python3
"""
Sync sprint-status.yaml from OpenProject work packages.
Fetches all epics and stories from OpenProject and updates sprint-status.yaml with current statuses.
"""
...
def fetch_work_packages_from_mcp():
    """
    This function should be called with MCP tool results.
    For now, returns structure that will be populated.
    """
    # This will be populated by MCP tool calls
    return []
...
def generate_sprint_status(work_packages, output_file):
    ...
    for wp in work_packages:
        ...
        status_id = wp.get("status", {}).get("id", 71) if isinstance(wp.get("status"), dict) else 71
        wp_type = wp.get("type", {}).get("name", "") if isinstance(wp.get("type"), dict) else ""
        
        # Skip test tasks and subtasks (tasks under stories)
