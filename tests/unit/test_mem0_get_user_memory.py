"""
Unit tests for mem0_get_user_memory MCP tool.

Tests cover:
- Memory retrieval from Mem0
- Memory retrieval from Redis fallback
...
from app.mcp.tools.memory_management import mem0_get_user_memory
...
class TestMem0GetUserMemory:
    """Tests for mem0_get_user_memory MCP tool."""
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_success_mem0(self):
        """Test successful memory retrieval from Mem0."""
        tenant_id = uuid4()
        user_id = uuid4()
        ...
        with patch("app.mcp.tools.memory_management.mem0_client") as mock_client:
            ...
            result = await mem0_get_user_memory(
                user_id=str(user_id),
            ...
            assert result["memories"][0]["memory_key"] == "preference_1"
            assert result["memories"][0]["source"] == "mem0"
            assert "response_time_ms" in result

    @pytest.mark.asyncio
    async def test_get_user_memory_redis_fallback(self):
        """Test memory retrieval falls back to Redis when Mem0 fails."""
        tenant_id = uuid4()
        user_id = uuid4()
        ...
        with patch("app.mcp.tools.memory_management.mem0_client") as mock_client:
            ...
            with patch("app.mcp.tools.memory_management.get_redis_client", return_value=mock_redis):
                ...
                assert len(result["memories"]) == 2
                assert result["memories"][0]["source"] == "redis_fallback"

    @pytest.mark.asyncio
    async def test_get_user_memory_filter_by_memory_key(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_access_validation_own_user(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_access_validation_tenant_admin(self):
        """Test that Tenant Admin can access any user's memories."""
        tenant_id = uuid4()
        user_id = uuid4()
        admin_user_id = uuid4()
        ...
        with patch("app.mcp.tools.memory_management.mem0_client") as mock_client:
            ...
            # Should not raise AuthorizationError (Tenant Admin can access)
            result = await mem0_get_user_memory(
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_access_validation_cross_user_denied(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_access_validation_cross_tenant_denied(self):
        """Test that users cannot access memories from other tenants."""
        tenant_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_invalid_user_id_format(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_invalid_tenant_id_format(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_with_filters(self):
        """Test memory retrieval with additional filters."""
        tenant_id = uuid4()
        user_id = uuid4()
        ...
        with patch("app.mcp.tools.memory_management.mem0_client") as mock_client:
            ...
            assert result["filtered_by"]["source"] == filters["source"]
            # Verify search_memory was called with filters
            mock_client.search_memory.assert_called_once()
            call_kwargs = mock_client.search_memory.call_args[1]
            assert call_kwargs.get("filters") is not None

    @pytest.mark.asyncio
    async def test_get_user_memory_response_time_tracking(self):
    ...
    @pytest.mark.asyncio
    async def test_get_user_memory_empty_results(self):
