...
if not tasks:
    raise Exception("No tasks found. PM must create tasks during grooming phase. Request PM to groom story first.")
```
...
tasks = [c for c in children["children"] if c["type"] == "Task"]
if not tasks:
    raise Exception("No tasks found. Request PM to groom story first.")
```
